...
const exchangeGuides: Record<string, ExchangeGuide> = {
	binance: {
		...
		notes: ["We do not support with importing Binance CSV file now."],
	},
	bybit: {
		name: "Bybit",
		steps: [
			...
			<div className="space-y-6">
							...
							<div className="space-y-3">
								<div>
									...
									<p className="text-sm text-blue-700 dark:text-blue-300">
										For the best experience, connect your
										exchanges first. This allows us to
										automatically sync your current balances
										and avoid duplicate transactions when
										importing CSV data.
									</p>
								</div>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Choose the exchange you want to import transaction
							history from
						</CardDescription>
					</CardHeader>
					<CardContent>
						...
						>
							...
							{Object.entries(exchangeGuides).map(
								([key, guide]) => (
									<TabsContent
										key={key}
										value={key}
				...
				{connectedExchanges.some(
					(ex: any) =>
						ex.name.toLowerCase() === selectedExchange &&
						ex.name.toLowerCase() !== "binance"
				) && (
					<Card>
						<CardHeader>
							<CardTitle className="flex items-center gap-2">
							...
							<CardDescription>
								Upload your {currentGuide.name} transaction
								history CSV file
							</CardDescription>
						</CardHeader>
						<CardContent>

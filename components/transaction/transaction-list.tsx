        ...
        <div className="space-y-1">
          {transByAsset?.length === 0 ? (
            ...
            <div className="space-y-3 sm:space-y-2">
              {transByAsset?.map((transaction: any, index: number) => (
                <TransactionRow
                  ...
                  mappingExchanges={mappingExchanges}
                  mappingExchangesLogo={mappingExchangesLogo}
                />
              ))}
            </div>
          )}
...
const TransactionRow = memo(function TransactionRow({
  ...
  if (isMobile) {
    return (
      <div className="space-y-4 py-4 px-4 bg-card rounded-xl shadow-sm border border-border/50">
        {/* Header Row */}
  ...
  return (
    <div className="grid grid-cols-12 items-center py-4 px-2 hover:bg-muted/30 rounded-lg transition-all duration-200 group gap-4">
      ...
      <div className="col-span-2 flex items-center space-x-2">
        <Avatar className="h-8 w-8 shrink-0">
          <AvatarImage
            ...
            alt={mappingExchanges(transaction.exchange_id)}
          />
          <AvatarFallback className="text-xs">{mappingExchanges(transaction.exchange_id).slice(0, 2)}</AvatarFallback>
        </Avatar>
